...
subset_data.16.1 <- subset_data.16[-8,]

# Select only ID = Control
subset_data.16.2 <- subset(subset_data.16.1, Group == "Control")
...
plot.16 <- ggplot(p_values_df, aes(x = PCB, y = -log10(p_value))) +
  ...
  labs(title = "SPME Control: AVL_S vs. AVL_NS 16 d", x = "",
       y = "-log10 (p-value)") +
  theme_bw() +
...
# Save plot in folder
ggsave("Output/Plots/StatAnalysis/SPME/PlotSPMEAVL_S_AVL_NS_Control_16d.png",
...
plot.35 <- ggplot(p_values_df, aes(x = PCB, y = -log10(p_value))) +
  geom_point() +
  geom_hline(yintercept = -log10(0.05), linetype = "dashed", color = "red") + # Adding a line at p = 0.05 for significance threshold
  labs(title = "SPME Control: AVL_S vs. AVL_NS 35 d", x = "",
       y = "-log10 (p-value)") +
  theme_bw() +
  theme(aspect.ratio = 10/15,
        axis.text.x = element_text(face = "bold", size = 10, angle = 60, hjust = 1,
...
# Save plot in folder
ggsave("Output/Plots/StatAnalysis/SPME/PlotSPMEAVL_S_AVL_NS_Control_35d.png",
...
plot.35 <- ggplot(p_values_df, aes(x = PCB, y = -log10(p_value))) +
  geom_point() +
  geom_hline(yintercept = -log10(0.05), linetype = "dashed", color = "red") + # Adding a line at p = 0.05 for significance threshold
  labs(title = "SPME Treatment: AVL_S vs. AVL_NS 35 d", x = "",
       y = "-log10 (p-value)") +
  theme_bw() +
  theme(aspect.ratio = 10/15,
        axis.text.x = element_text(face = "bold", size = 10, angle = 60, hjust = 1,
...
# Save plot in folder
ggsave("Output/Plots/StatAnalysis/SPME/PlotSPMEAVL_S_AVL_NS_Treatment_35d.png",
       plot = plot.35, width = 15, height = 10, dpi = 300)

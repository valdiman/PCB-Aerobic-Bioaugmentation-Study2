...
plot.3 <- ggplot(p_values_df, aes(x = PCB, y = -log10(p_value))) +
  ...
  labs(title = "SPME Control vs Treatment: AVL 3 d", x = "",
       y = "-log10 (p-value)") +
  theme_bw() +
...
# Save plot in folder
ggsave("Output/Plots/StatAnalysis/SPME/PlotSPMEAVL_S_3d.png", plot = plot.3, width = 15,
       height = 10, dpi = 300)
...
plot.11 <- ggplot(p_values_df, aes(x = PCB, y = -log10(p_value))) +
  geom_point() +
  geom_hline(yintercept = -log10(0.05), linetype = "dashed", color = "red") + # Adding a line at p = 0.05 for significance threshold
  labs(title = "SPME Control vs Treatment: AVL 11 d", x = "",
       y = "-log10 (p-value)") +
  theme_bw() +
  theme(aspect.ratio = 10/15,
        axis.text.x = element_text(face = "bold", size = 10, angle = 60, hjust = 1,
...
# Save plot in folder
ggsave("Output/Plots/StatAnalysis/SPME/PlotSPMEAVL_S_11d.png", plot = plot.11, width = 15,
       height = 10, dpi = 300)
...
plot.35 <- ggplot(p_values_df, aes(x = PCB, y = -log10(p_value))) +
  geom_point() +
  geom_hline(yintercept = -log10(0.05), linetype = "dashed", color = "red") + # Adding a line at p = 0.05 for significance threshold
  labs(title = "SPME Control vs Treatment: AVL 35 d", x = "",
       y = "-log10 (p-value)") +
  theme_bw() +
  theme(aspect.ratio = 10/15,
        axis.text.x = element_text(face = "bold", size = 10, angle = 60, hjust = 1,
...
# Save plot in folder
ggsave("Output/Plots/StatAnalysis/SPME/PlotSPMEAVL_S_35d.png", plot = plot.35, width = 15,
       height = 10, dpi = 300)

...
# NBH_NS 16 days Analysis ---------------------------------------------------

# Filter the data for the specific Time and ID
subset_data.16 <- subset(PCB_data, Time == 16 & ID == "NBH_NS")
...
plot.16 <- ggplot(p_values_df, aes(x = PCB, y = -log10(p_value))) +
  ...
  labs(title = "SPME Control vs Treatment: NBH_NS 16 d", x = "",
       y = "-log10 (p-value)") +
  theme_bw() +
...
# Save plot in folder
ggsave("Output/Plots/StatAnalysis/SPME/PlotSPMENBH_NS_16d.png", plot = plot.16, width = 15,
       height = 10, dpi = 300)

# NBH_NS 35 days Analysis ---------------------------------------------------

# Filter the data for the specific Time and ID
subset_data.35 <- subset(PCB_data, Time == 35 & ID == "NBH_NS")
...
plot.35 <- ggplot(p_values_df, aes(x = PCB, y = -log10(p_value))) +
  geom_point() +
  geom_hline(yintercept = -log10(0.05), linetype = "dashed", color = "red") + # Adding a line at p = 0.05 for significance threshold
  labs(title = "SPME Control vs Treatment: NBH_NS 35 d", x = "",
       y = "-log10 (p-value)") +
  theme_bw() +
  theme(aspect.ratio = 10/15,
        axis.text.x = element_text(face = "bold", size = 10, angle = 60, hjust = 1,
...
# Save plot in folder
ggsave("Output/Plots/StatAnalysis/SPME/PlotSPMENBH_NS_35d.png", plot = plot.35, width = 15,
       height = 10, dpi = 300)

# NBH_NS 75 days Analysis ---------------------------------------------------

# Filter the data for the specific Time and ID
subset_data.75 <- subset(PCB_data, Time == 75 & ID == "NBH_NS")
...
plot.75 <- ggplot(p_values_df, aes(x = PCB, y = -log10(p_value))) +
  geom_point() +
  geom_hline(yintercept = -log10(0.05), linetype = "dashed", color = "red") + # Adding a line at p = 0.05 for significance threshold
  labs(title = "SPME Control vs Treatment: NBH_NS 75 d", x = "",
       y = "-log10 (p-value)") +
  theme_bw() +
  theme(aspect.ratio = 10/15,
        axis.text.x = element_text(face = "bold", size = 10, angle = 60, hjust = 1,
...
# Save plot in folder
ggsave("Output/Plots/StatAnalysis/SPME/PlotSPMENBH_NS_75d.png", plot = plot.75, width = 15,
       height = 10, dpi = 300)

...
# (i) Fit the exponential desorption model for PCB_1 to PCB_99 and extract k, M_w0, and R²
...
ggplot(k_logKow_df, aes(x = logKow, y = k)) +
  ...
  labs(title = "Regression of k against logKow (PCB1 - PCB99)",
       x = "logKow",
       y = "Desorption rate k") +
  theme_minimal()

# (ii) Fit the exponential desorption model for PCB_4 to PCB_99 and extract k, M_w0, and R²
model_results <- df_long_filtered %>%
  filter(grepl("^PCB_([4-9]|[1-9][0-9])$", PCB)) %>%  # Filter for PCB_4 to PCB_99
  group_by(PCB) %>%
  do({
    model <- nls(Concentration ~ M_w0 * exp(-k * time), 
                 data = ., 
...
ggplot(k_logKow_df, aes(x = logKow, y = k)) +
  geom_point() +                      # Scatter plot of k vs logKow
  geom_smooth(method = "lm", se = TRUE, color = "blue") +  # Add regression line
  labs(title = "Regression of k against logKow (PCB4-PCB99)",
       x = "logKow",
       y = "Desorption rate k") +
  theme_minimal()

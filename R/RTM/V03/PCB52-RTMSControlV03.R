...
rtm.PCB52 = function(t, state, parms){
  ...
  ko <- 4 # cm/d PCB 52 mass transfer coefficient to SPME
  ...
  bl <- 1 # cm boundary layer thickness
  ...
  # Particle adsorption/desoprtion
  ka <- 0.089
  kdes <- 0.00036
  ...
  dCpwdt <- -kb * Cpw
  dCwdt <- kaw.o * Aaw / Vw * (Ca / (Kaw) - Cw) + Cpw * (kdes * M *  Kd * Vpw / Vw - ka) - kb * Cw # 864 to change second to days and um to m, Ca in [ng/L]
...
out.1 <- ode(y = cinit, times = t.1, func = rtm.PCB52)
head(out.1)
...
mf_r2_value <- mf_r2(comparison_data$mf, comparison_data$mf_Control)
mpuf_r2_value <- mf_r2(comparison_data$mpuf, comparison_data$mpuf_Control)

# Print R-squared values
print(paste("R-squared for mf: ", mf_r2_value))
print(paste("R-squared for mpuf: ", mpuf_r2_value))

# Plot
# Run the model with the new time sequence
cinit <- c(Cpw = Cpw, Cw = 0, mf = 0, Ca = 0, mpuf = 0)
t_daily <- seq(0, 50, by = 1)  # Adjust according to your needs
out_daily <- ode(y = cinit, times = t_daily, func = rtm.PCB52)
...
# Save plot in folder
ggsave("Output/Plots/RTM/PCB52ALV_NS_Control.png", plot = p.52, width = 6,
       height = 5, dpi = 500)

...
objective_function <- function(params) {
  ...
  parms <- list(ro = ro, ko = ko, kdf = kdf, kds = kds, ka = ka, f = 0.8, kb = 0)
  
  # Solve ODE
  out <- ode(y = cinit, times = t.1, func = rtm.PCB19, parms = parms)
...
initial_params <- c(ro = 540, ko = 10, kdf = 2, kds = 0.001, ka = 150)

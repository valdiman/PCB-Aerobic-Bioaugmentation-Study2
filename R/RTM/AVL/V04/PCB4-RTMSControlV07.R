...
rtm.PCB4 = function(t, state, parms){
  ...
  # Sortion, desorption and biotransformation rates
  ka <- parms$ka #1/d
  kd <- parms$kd #1/d
  kb <- parms$kb
  ...
  # Derivatives
  dCwdt <- (kaw.o * Aaw / Vw * (Ca / (Kaw) - Cw) + kd * Cw * K / Vw - ka * Cw / Vw - kb * Cw) / B # 864 to change second to days and um to m, Ca in [ng/L]
...
# Constants for initial calculation of Cwi
Cto <- 307.3052312 * 4  # Initial sediment concentration [ng/g]
...
# Compute initial Cwi using Cto at time t = 0
Cwi <- Cto * M * 1000 / (1 + M * K)

# Initial conditions for the ODE
cinit <- c(Cw = Cwi, mf = 0, Ca = 0, mpuf = 0)

# Parameters (including kb for Ct decay)
parms <- list(ka = 30, kd = 0.05, kb = 0.002)  # kb is the degradation rate constant

# Time points to simulate (from pcb_combined_control$time)

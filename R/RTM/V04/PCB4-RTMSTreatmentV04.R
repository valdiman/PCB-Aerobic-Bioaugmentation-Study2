...
rtm.PCB4 = function(t, state, parms){
  ...
  ro <- 0.0002 # m3/d sampling rate 0.004, 0.00025
  ...
  kb <- 0.65 #23 # 1/d, value changes depending on experiment 0.023 from SPME calibration
  ...
  dCwdt <- kaw.o * Aaw / Vw * (Ca / (Kaw) - Cw) + Cw * kd * M * K * Vpw / Vw - ka * Cw - kb * Cw # 864 to change second to days and um to m, Ca in [ng/L]
...
Ct <- 630.2023 * 1.2  # ng/g PCB 4 sediment concentration
...
parms <- list(ka = 0.089, kd = 0.0007) #Input reasonable estimate of ka and kd (placeholder values) kd = 0.000036, 0.0005, ka = 731.611, kd = 15.782ka = 0.089, kd = 0.000036
...
mf_r2_value <- mf_r2(comparison_data$mf, comparison_data$mf_Treatment)
mpuf_r2_value <- mf_r2(comparison_data$mpuf, comparison_data$mpuf_Treatment)

...
# Organize data -----------------------------------------------------------
{
  ...
  pcbi.spme.control <- pcbi %>%
    filter(ID == "AVL_NS", Group == "Control", Sample_medium == "SPME") %>%
...
parms <- list(ro = 500, ko = 6, kb = 0.3) # Input
...

{
  ...
  # Export data
  write.csv(model_results_daily_clean, file = "Output/Data/RTM/NS/AVL/PCB4AVLSControlFV.csv")
...
# Save plot in folder
ggsave("Output/Plots/RTM/NS/AVL/PCB4ALV_NS_ControlFV.png", plot = p.4, width = 6,
       height = 5, dpi = 500)

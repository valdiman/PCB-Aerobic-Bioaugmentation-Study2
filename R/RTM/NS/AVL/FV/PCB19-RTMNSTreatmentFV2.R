...
rtm.PCB19 = function(t, state, parms){
  ...
  Kf <- 10^(1.06 * log10(Kow.t) - 1.16) # PCB 19-SPME equilibrium partition coefficient [Lw/Lf]
  ...
  # Time points for Clb400
  time_points <- c(0, 16, 35, 75)   # Time in days
  Clb400_values <- c(0.8 * 8 * 10^8, 0.8 * 8 * 10^3.7, 0.8 * 8 * 10^3.1,
                     0.8 * 8 * 10^2.56) # [cell/mL]
  # Create an interpolation function for Clb400
  Clb400_interp <- approxfun(time_points, Clb400_values, rule = 2)
  Clb400_t <- Clb400_interp(t)
  Vlb400 <- 1 # [um3/cell]
  Mlb400 <- Clb400_t * Vlb400 * 10^-12# [Llb400/Lw]
...

{
  ...
  # Export data
  #write.csv(model_results_daily_clean, file = "Output/Data/RTM/NS/AVL/PCB19AVLSTreatmentFV2.csv")
...
# Save plot in folder
ggsave("Output/Plots/RTM/NS/AVL/PCB19ALV_NS_TreatmentFV2.png", plot = p.19, width = 6,
       height = 5, dpi = 500)

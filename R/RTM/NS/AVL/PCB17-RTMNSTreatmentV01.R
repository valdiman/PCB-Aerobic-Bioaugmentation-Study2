...
rtm.PCB17 = function(t, state, parms){
  ...
  Kpuf <- 10^(0.6366 * log10(Koa) - 3.1774)# m3/g PCB 17-PUF equilibrium partition coefficient
  ...
  D.pcb.air <- D.water.air*(MW.pcb/MH2O)^(-0.5) # cm2/s PCB 17's diffusion coefficient in the gas phase (eq. 18-45)
  D.pcb.water <- D.co2.w*(MW.pcb/MCO2)^(-0.5) # cm2/s PCB 17's diffusion coefficient in water @ Tair = 25 C, patm = 1013.25 mbars
  ...
  SC.pcb.w <- v.H2O/D.pcb.water # Schmidt number PCB 17
  ...
  # Biotransformation rate and time
  kb <- parms$kb
  tb1 <- parms$tb1
  tb2 <- parms$tb2
...
Ct <- 307.3052312 * 10  # ng/g PCB 17 sediment concentration
...
parms <- list(ro = 0.0004, ko = 10, tb1 = 20, tb2 = 35, kb = 0.01) # Input
...
# Export data
#write.csv(model_results_daily_clean, file = "Output/Data/RTM/NS/AVL/PCB17NSTreatment.csv")
...
# Save plot in folder
ggsave("Output/Plots/RTM/NS/AVL/PCB17ALV_NS_Treatment.png", plot = p.17, width = 15,
       height = 5, dpi = 500)

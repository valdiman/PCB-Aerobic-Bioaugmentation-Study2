...
rtm.PCB19 = function(t, state, parms){
  ...
  dCwdt <- kaw.o * Aaw / Vw * (Cw - Ca / Kaw.t) -
    ko * Af * L / Vw * (Cw - Cf / Kf) -
    kb * Cw # [ng/L]
  ...
  return(list(c(dCwdt, dCfdt, dCadt, dCpufdt)))
}
...
cinit <- c(Cw = Cw, Cf = 0, Ca = 0, Cpuf = 0)
...
head(out.1)
  ...
  colnames(out.1) <- c("time", "Cw", "Cf", "Ca", "Cpuf")
  ...
  out.1$Mt <- out.1$Cw * Vw / 1000 + out.1$Cf * Vf + out.1$Ca * Va / 1000 + out.1$Cpuf * Vpuf / 1000
  ...
  # Export data
  #write.csv(model_results_daily_clean, file = "Output/Data/RTM/NS/AVL/PCB19AVLSControl.csv")
...
# Save plot in folder
ggsave("Output/Plots/RTM/NS/AVL/PCB19ALV_NS_Control.png", plot = p.19, width = 6,
       height = 5, dpi = 500)

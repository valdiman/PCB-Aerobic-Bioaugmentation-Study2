...
rtm.PCB17 = function(t, state, parms){
  ...
  Koa <- 10^(7.628540115) # PCB 17 octanol-air equilibrium partition coefficient @ 25 C
...
parms <- list(ro = 500, ko = 1, kb = 1) # Input
...

{
  ...
  out_daily <- ode(y = cinit, times = t_daily, func = rtm.PCB17,
                   parms = parms)
  head(out_daily)

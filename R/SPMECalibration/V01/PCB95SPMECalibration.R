...
Kd <- foc * 10^(logKoc) * 0.60 # [L/kg] added a calibration factor to the Kd
Cpw <- Ct / Kd * 1000 # [ng/L]
...
# Observed values
observed <- pcbi$PCB95 / pcbi$length
...
p.95 <- ggplot() +
  ...
  theme_bw() +
  labs(x = "Time [day]", y = "PCB 95 [ng/cm]") +
...
# Save plot in folder
ggsave("Output/Plots/SPMECalibration/PCB95V01.png", plot = p.95, width = 6,
       height = 5, dpi = 500)

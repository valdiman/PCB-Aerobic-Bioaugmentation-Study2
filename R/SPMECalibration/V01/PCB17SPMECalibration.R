...
Kd <- foc * 10^(logKoc) * 0.3  # [L/kg] added a calibration factor to the Kd
Cpw <- Ct / Kd * 1000  # [ng/L]
...
times <- seq(0, 100, by = 1)  # Make sure t = 0 is included
...
initial_params <- c(kb = 0.05, kf = 0.1)
...
optimized_kb <- optim_result$par[1]
optimized_kf <- optim_result$par[2]
...
plot_data <- data.frame(time = solution_df$time, mf = solution_df$mf)
...
# Save plot in folder
ggsave("Output/Plots/SPMECalibration/PCB17V01.png", plot = p.17, width = 6,
       height = 5, dpi = 500)

...
kf <- 0.5  # [cm/d] MTC from fiber/water

# Calculate Cpw
Ct <- 307.3052312 * 1.7  # [ng/g]
foc <- 0.03
logKoc <- 0.94 * log10(Kow) + 0.42
Kd <- foc * 10^(logKoc)  # [L/kg] added a calibration factor to the Kd
Cpw <- Ct / Kd * 1000  # [ng/L]

kb <- 0.01 # [1/d] biotransformation rate
...
# Observed values
observed <- pcbi$PCB17 / pcbi$length
...
p_mf <- ggplot() +
  ...
  theme_bw() +
  labs(x = "Time [day]", y = "PCB 17 [ng/cm]") +
...
# Save plot in folder
ggsave("Output/Plots/SPMECalibration/V02/PCB17V02.png", plot = p_mf, width = 6,
       height = 5, dpi = 500)

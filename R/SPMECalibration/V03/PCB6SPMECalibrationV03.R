...
kf <- 80  # [cm/d] MTC from fiber/water

# Calculate Cpw
Ct <- 80.6621058 # [ng/g]
foc <- 0.03
logKoc <- 0.94 * log10(Kow) + 0.42
Kd <- foc * 10^(logKoc) * 0.5 # [L/kg] added a calibration factor to the Kd
Cpw <- Ct / Kd * 1000  # [ng/L]

kb <- 0.1 # [1/d] biotransformation rate
...
# Solve the system of equations with the manually set kf
solution <- ode(y = initial_conditions, 
                times = times, 
                func = system_equations, 
                parms = NULL,  # Still needs to pass parms, even if NULL
                method = "lsoda")
...
plot_data$Cpw <- approx(solution_df$time, solution_df$Cpw, xout = plot_data$time)$y

# Observed values
observed <- pcbi$PCB6 / pcbi$length
...
# Plot Cpw
p_Cpw <- ggplot() +
  geom_line(data = plot_data, aes(x = time, y = Cpw), color = "green", size = 1.5) +
  theme_bw() +
  labs(x = "Time [day]", y = "Cpw [ng/L]") +
  theme(aspect.ratio = 1,
        panel.border = element_blank(),
        axis.line = element_line(size = 0.5),
        axis.line.x = element_line(color = "black"),
...
grid.arrange(p_mf, p_Cpw, ncol = 2)

# Print the plots
print(p_mf)

# Save plot in folder
ggsave("Output/Plots/SPMECalibration/V02/PCB6V02.png", plot = p_mf, width = 6,
       height = 5, dpi = 500)

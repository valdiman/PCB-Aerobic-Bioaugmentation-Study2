...
kf <- 0.1  # [cm/d] MTC from fiber/water

# Calculate Cpw
Ct <- 259.8342356 * 3.5 # [ng/g]
foc <- 0.03
...
initial_conditions <- c(mf = 0)  # Initial condition for mf and Cw
...
# Observed values
observed <- pcbi$PCB19 / pcbi$length
...
rolling_sd_interp <- approx(time_non_na, rolling_sd_non_na,
                            xout = seq_along(plot_data$time), rule = 2)$y
...
p_mf <- ggplot() +
  ...
  theme_bw() +
  labs(x = "Time [day]", y = "PCB 19 [ng/cm]") +
...
# Save plot in folder
ggsave("Output/Plots/SPMECalibration/V02/PCB19V03.png", plot = p_mf, width = 6,
       height = 5, dpi = 500)

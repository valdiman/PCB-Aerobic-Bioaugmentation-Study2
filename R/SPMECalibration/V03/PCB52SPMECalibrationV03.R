...
kf <- 4  # [cm/d] MTC from fiber/water

# Calculate Cpw
Ct <- 321.4900673 * 1.5 # [ng/g]
foc <- 0.03
...
system_equations <- function(t, state, parms) {
  ...
  dmfdt <- (kf * Af / (L * 1000) * (Cpw - mf / (Kfi * Vf * L)))  # [ng/cmf/d]
...
# Plot using ggplot2
p.52 <- ggplot() +
  geom_line(data = plot_data, aes(x = time, y = mf), color = "blue", linewidth = 1.5) +  # Use linewidth instead of size
  ...
  theme(aspect.ratio = 1,
        ...
        axis.line = element_line(linewidth = 0.5),  # Use linewidth instead of size
...
# Save plot in folder
ggsave("Output/Plots/SPMECalibration/V02/PCB52V02.png", plot = p.52, width = 6,
       height = 5, dpi = 500)

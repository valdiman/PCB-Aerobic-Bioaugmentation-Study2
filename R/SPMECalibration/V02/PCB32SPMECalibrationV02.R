...
# Observed values
observed <- pcbi$PCB32 / pcbi$length

# Initial guess for kf
initial_kf <- 0.25  # Starting value for kf
...
p_mf <- ggplot() +
  ...
  theme_bw() +
  labs(x = "Time [day]", y = "PCB 32 [ng/cm]") +
...
# Save plot in folder
ggsave("Output/Plots/SPMECalibration/PCB32V01.png", plot = p_mf, width = 6,
       height = 5, dpi = 500)

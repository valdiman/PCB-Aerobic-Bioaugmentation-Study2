...
SedWatSPMEAirPufV01 = function(t, state, parms){
  ...
  dCwdt <- (- ka * Cw + f * kdf * Cs + (1 - f) * kds * Cs -
              ...
              ko * Af * L / Vw * (Cw - mf * 1000 / (Vf * Kf)) -
              kb * Cw) / B # [ng/L]
  dmfdt <- (ko * Af / 1000 * (Cw - mf * 1000 / (Vf * Kf))) / B # Cw = [ng/L], mf = [ng/cmf]
  dCadt <- (kaw.o * Aaw / Va * (Cw - Ca / Kaw.t) -
              ro * Apuf / Va * (Ca - mpuf / (Vpuf * d * Kpuf * 1000)))/ B # Ca = [ng/L]
  dmpufdt <- (ro * Apuf / Vpuf * (Ca - mpuf / (Vpuf * d * Kpuf * 1000))) / B # Ca = [ng/L], mpuf = [ng/L]
  ...
  
{
  ...
  df.3$Mf <- df.3$mf * l # [ng]
  df.3$Ma <- df.3$Ca * Va / 1000 # [ng]
  df.3$Mpuf <- df.3$mpuf * Vpuf / 1000 # [ng]
  df.3$Mt <- df.3$Cs * ms / (M * 1000) + df.3$Cw * Vw / 1000 + df.3$Ca * Va / 1000 + df.3$mf * l + df.3$mpuf * Vpuf / 1000 # [ng]

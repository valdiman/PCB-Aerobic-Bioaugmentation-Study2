...
SedWatSPMEAirV01 = function(t, state, parms){
  ...
  dCwdt <- (- ka * Cw + f * kdf * Cs + (1 - f) * kds * Cs -
              ...
              ko * Af * L / Vw * (Cw - Cf / Kf) -
              kb * Cw) / B # [ng/L]
  dCfdt <- (ko * Af / Vf * (Cw - Cf / Kf)) / B # Cw = [ng/L], mf = [ng/L]
  dCadt <- (kaw.o * Aaw / Va * (Cw - Ca / Kaw.t))/ B # Ca = [ng/L]
  
  # The computed derivatives are returned as a list
  return(list(c(dCsdt, dCwdt, dCfdt, dCadt)))
}
...

{
  ...
  colnames(df.2) <- c("time", "Cs", "Cw", "Cf", "Ca")
  ms <- 10 # [g]
  M <- 0.1 # kg/L solid-water ratio
  Vw <- 100 # [cm3]
  Va <- 125 # [cm3]
  l <- 1 # [cm]
  Vf <- 0.000000069 * 1000 # cm3/cm SPME volume/area
  df.2$Mp <- df.2$Cs * ms / (M * 1000)
  df.2$Mw <- df.2$Cw * Vw / 1000
  df.2$Mf <- df.2$Cf * Vf / 1000
  df.2$Ma <- df.2$Ca * Va / 1000
  df.2$Mt <- df.2$Cs * ms / (M * 1000) + df.2$Cw * Vw / 1000 + df.2$Cf * Vf / 1000  + df.2$Ca * Va / 1000 # [ng]

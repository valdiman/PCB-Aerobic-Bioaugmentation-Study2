...
SedWatAirV01 = function(t, state, parms){
  ...
  Vf <- 0.000000069 * 1000 # cm3/cm SPME volume/cm
  L <- 1 # cm SPME length normalization to 1 cm
  Kf <- 10^(1.06 * log10(Kow.t) - 1.16) # PCB 4-SPME equilibrium partition coefficient
  ...
  dCsdt <- - f * kdf * Cs - (1 - f) * kds * Cs + ka * Cw * K * M
  dCwdt <- f * kdf * Cs + (1 - f) * kds * Cs - ka * Cw + 
...

{
  ...
  df.1$Ms <- df.1$Cs * Vw / 1000 # = Cs * ms / M / 1000 [ng]
  df.1$Mw <- df.1$Cw * Vw / 1000
  df.1$Ma <- df.1$Ca * Va / 1000
  df.1$Mt <- df.1$Ms + df.1$Mw + df.1$Ma  # [ng]
  df.1$fp <- df.1$Ms / df.1$Mt * 100 # [ng]
  df.1$fw <- df.1$Mw / df.1$Mt * 100 # [ng]
  df.1$fa <- df.1$Ma / df.1$Mt * 100 # [ng]
}

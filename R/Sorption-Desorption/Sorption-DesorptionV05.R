...
dCwdt <- (- ka * Cw + f * kdf * Cs + (1 - f) * kds * Cs + kaw.o * Aaw / Vw * (Ca / (Kaw.t) - Cw) - kb * Cw) / B # [ng/L]
...
parms <- list(kdf = 5, kds = 0.1, f = 0.6,
              ka = 450, kb = 0) # Input 
t <- seq(from = 0, to = 40, by = 1)
# Run the ODE function without specifying parms
out.3 <- ode(y = cinit, times = t, func = SorptionDesorptionV02, parms = parms)
head(out.3)

{
  ...
  df.3$Mt <- (df.3$Cs + df.3$Cw) * Vw / 1000 + df.3$Ca * Va / 1000 # [ng]
}
